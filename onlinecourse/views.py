...
def submit_exam(request, course_id):
    ...
    # If the request is GET, render the exam submission form
    return render(request, 'submit_exam.html', {'course': course})
...
def show_exam_result(request, course_id, submission_id):
    ...
    passed_exam = score >= course.passing_score
    ...
    return render(request, 'exams/show_exam_result.html', context)
...
def show_exam_result(request, course_id, submission_id):
    ...
    # Get the selected choice ids from the submission record
    selected_ids = [int(i) for i in submission.choices.split(',')]
    
    # Initialize total score and a list of dictionaries to store question results
    total_score = 0
    question_results = []
    ...
    context = {
        ...
        'passed_exam': total_score >= total_grade / 2
    }
